/* ---------------------------------------------------------------
   Mapa de Ocupacao de Assentos no Cinema (foco: MATRIZES 2D)
   - 0 = livre, 1 = ocupado
   - Conteudos: declaracao, indexacao [linha][coluna], laços aninhados,
                soma de elementos, busca, preenchimento manual/auto,
                e (opcional) transposicao.
   - Menu com if...else (sem "viajar").
---------------------------------------------------------------- */
#include <stdio.h>    // printf, scanf
#include <stdlib.h>   // rand, srand
#include <time.h>     // time (semente para rand)

/* Tamanho didático (poucos assentos, fácil de visualizar na tela) */
#define LINHAS 5
#define COLUNAS 6
...
int main(void) {
    ...
    while (opcao != 0) {
        ...
        if (opcao == 1) {
            ...
            /* OPCIONAL: transpor a sala para reforçar conteúdo de aula */
            transporSala(sala, T);
